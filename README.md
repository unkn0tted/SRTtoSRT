...
| 任务管理 | 支持多文件队列、并发、进度显示、软停止和日志记录 |
| 容错 | 输出目录旁生成 checkpoint，异常、停止后都可继续跑 |
...
- 正在执行的一轮会冻结当时的参数快照
- 运行中继续修改表单，只会影响下一次“开始翻译”或“继续未完成任务”
- 如果修改了 `Model / Prompt / 英文模式 / 额外请求头 / API Base URL` 这类会影响输出内容的参数，旧 checkpoint 可能会失效并从头重跑对应文件
- 如果只修改 `并发 / 超时 / 批大小 / 字符上限` 这类执行参数，通常可以继续利用已有 checkpoint
...
- 当前是软停止，不会强杀已经发出的 HTTP 请求；会等待当前批次自然收尾后再暂停
- 断点续跑目前只适用于同一输入文件、同一输出目录、同一套输出内容参数
- 对复杂字幕标签暂时没有专门保护逻辑
- 没有内置术语表和上下文记忆
